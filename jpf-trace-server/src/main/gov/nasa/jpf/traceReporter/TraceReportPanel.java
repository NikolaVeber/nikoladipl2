...
public class TraceReportPanel extends ShellPanel implements
    VerifyCommandListener {
  ...
  public TraceReportPanel() {
    super("Trace Report", null, "View JPF's Output");
    ...
    splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, listScroll,
        textScroll);
          ...
          Runnable saveFile = new Runnable() {
            public void run() {
              try {
                ...
                String time = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss z")
                    .format(new Date());
                out.print("Created: ");
                out.println(time);
                ...
                for (Entry<String, Topic> e : topicListModel.getTopics()
                    .entrySet())
                ...
                getShell().error(
                    "File: " + file.getName() + " could not created.");
                return;
              }
            }
          };
   ...
   * {@link gov.nasa.jpf.traceReporter.TopicPublisher TopicPublisher}s for every
   * trace printer defined (see
  ...
  public void preCommand(final VerifyCommand command) {
    ...
    if (publishers.contains("genericConsoleTracePrinter")) {
      config.put("report.publisher", config.getProperty("report.publisher", "")
          + ",genericConsoleTopic");
      config.put("report.genericConsoleTopic.class",
          GenericConsoleTopicPublisher.class.getName());
    }
    if (publishers.contains("consoleTracePrinter")
        || !publishers.contains("genericConsoleTracePrinter")) {
      config.put("report.publisher", config.getProperty("report.publisher", "")
          + ",consoleTopic");
      config.put("report.consoleTopic.class",
          ConsoleTopicPublisher.class.getName());
    }
    tracker.resetFields();
  }
  ...
  public void postCommand(VerifyCommand command) {
    ...
    if (command.errorOccured()) {
      statusLabel
          .setText("An Error occured during the verify, check the Error Panel for more details");
