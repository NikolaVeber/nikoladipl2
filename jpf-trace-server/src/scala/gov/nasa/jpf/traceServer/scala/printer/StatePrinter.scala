...
import gov.nasa.jpf.traceServer.traceStorer.EventTypes.eventType._
...
class StatePrinter (conf: Config, tracePrinterPrefix: String, name: String)
  extends EventPrinter(conf, tracePrinterPrefix, name) {
  
  override def print(out: PrintWriter, event: Event) {
    if (event.getEventType() == stateAdvanced) {
      out.print("====================================================== stateId: "
          + event.getProperty(STATE_ID));
    } else {
