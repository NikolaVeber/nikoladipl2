...
public class CoverageAnalyzer extends PropertyListenerAdapter implements PublisherExtension {
  ...
  static class PathMethodCoverage {
	  /*
	   * Traces the coverage for each segment of the path
	   * after each state progress, new coverage is initiated, and after backtrack, it is "removed", so that only the coverage of still used segments of the path is considered
	   */
