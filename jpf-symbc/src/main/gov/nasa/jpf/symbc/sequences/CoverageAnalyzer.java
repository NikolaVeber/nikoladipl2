 ...
 */
public class CoverageAnalyzer extends PropertyListenerAdapter implements PublisherExtension {

  static Logger log = JPF.getLogger("gov.nasa.jpf.symbc.sequences.CoverageAnalyzer");
  ...
  static class MethodCoverage {
    ...
    void setExecuted(ThreadInfo ti, Instruction insn) {
    	int idx = ti.getId();
    	setExecuted(idx, insn);
    }
    
    void setExecuted(int idx, Instruction insn) {
    ...
    void unSetExecuted(int idx, Instruction insn) {
        ...
        if (showBranchCoverage && (insn instanceof IfInstruction)) {
          ...
          if (!((IfInstruction) insn).getConditionValue()) {
            branchTrue.clear(off);
          } else {
            branchFalse.clear(off);
          }
        }
      }
    ...
    Coverage getCoveredInsnDiff(MethodCoverage mc) {
        int nTotal = mi.getInstructions().length;
        ...
        if (covered != null) {
          ...
          BitSet bExecDiff = filterBitsetDiff(bExec, mc.getExecutedInsn());
          
          return new Coverage(nTotal-mc.getExecutedInsn().length(), bExecDiff.cardinality());
        } else {
        	BitSet bExec = new BitSet(0);
        	BitSet result = filterBitsetDiff(bExec, mc.getExecutedInsn());
        	return new Coverage(result.length(), result.cardinality());
        }
      }
    
    
    /*
     * Return those bits from the first parameter, which are 0 in the second parameter BitSet
     * 
     * 1	0
     * 0	0		0
     * 0 x  1  -> 	1
    ...
    private BitSet filterBitsetDiff(BitSet bExec, BitSet filter){

    	BitSet result = new BitSet(filter.length() - filter.cardinality()); // we are interested in zero values
    	int j = 0;
    	for(int i = 0; i < filter.length(); i++){
    		if(!filter.get(i)){ // if the bit in the filter is not set, copy the bit from the object
    			if(bExec.get(i)){
    				result.set(j);
    			}
    			j++;
    ...
    Coverage getCoveredLines() { // called from print methods
    ...
    boolean getCoveredLines(BitSet executable, BitSet covered) { // called from MethodCoverage and ClassCoverage: getCoveredLines()
    ...
    Coverage getCoveredBranches() { // calleds from MethodCoverage::getCoveredBranches and publishers 
    ...
    Coverage getCoveredBasicBlocks() { // called from ClassCoverage:getCoveredBasicBlocks, publishers
      BitSet bExec = getExecutedInsn();
      BitSet bb = getBasicBlocks();
      int nCov = 0;
  ...
  static class ClassCoverage {
    ...
    MethodCoverage getMethodCoverage(MethodInfo mi) { // called from setExecuted
      if (methods == null) {
        setLoaded(ci);
      }
    ...
    Coverage getCoveredMethods() { // called from computeCoverages() and publisher methods
    ...
    Coverage getCoveredInsn() { // called from computeCoverages() and publisher methods
      Coverage cov = new Coverage(0, 0);
    ...
    Coverage getCoveredInsnDiff(ClassCoverage cc) { 
      Coverage cov = new Coverage(0, 0);
      if (methods != null) {
        for (MethodInfo mi : methods.keySet()) {
        	MethodCoverage mc = methods.get(mi);
        	MethodCoverage mc2 = cc.getMethodCoverage(mi);
        	
          Coverage c = mc.getCoveredInsnDiff(mc2);
          cov.total += c.total;
          cov.covered += c.covered;
        }
      }
    ...
    boolean getCoveredLines(BitSet executable, BitSet covered) { // called from ClassCoverage.getCoveredLines() and publisher methods
    ...
    Coverage getCoveredLines() { // called from computeCoverages() and publisher methods
      BitSet executable = new BitSet();
      BitSet covered = new BitSet();
    ...
    Coverage getCoveredBasicBlocks() { // called from computeCoverages() and publisher methods
      Coverage cov = new Coverage(0, 0);
    ...
    Coverage getCoveredBranches() {  // called from computeCoverages() and publisher methods
      Coverage cov = new Coverage(0, 0);
  ...
  public void instructionExecuted(Instruction insn, int idx) {
	    MethodCoverage mc = getMethodCoverage(insn);

	    if (mc != null) {
	      mc.setExecuted(idx, insn);
  ...
  public void instructionDeExecuted(Instruction insn, int idx) {
	    MethodCoverage mc = getMethodCoverage(insn);

	    if (mc != null) {
	      mc.unSetExecuted(idx, insn);
